...
public class DataTransferUtil {
   ...
   * .
   *
   * @Description 将6位的byte数组转换成yyMMddhhmmss的时间，年份为1位byte
   * @notice
   */
  public static String byteArrayToDateYear(byte[] bytes) {
    ByteBuffer buffer = ByteBuffer.wrap(bytes);
    return "" + String.format("%02x", bytes[0]) + String.format("%02x", bytes[1])
        + String.format("%02x", bytes[2]) + String.format("%02x", bytes[3]) + String
        .format("%02x", bytes[4])
   ...
   * .
   *
   * @param signalInstallationCross 安装路口
   * @param length 长度
   */
        ...
        /*for (int j = 0; j <= 8; j++) {
          if (quotients == j) {
            if (mod
                == 0) {
              temp = 8;
              sum[8 - j] += Math.pow(2, temp - 1);
            } else {
              temp = mod;
              sum[7 - j] += Math.pow(2, temp - 1);
            }
          }
        }*/
   ...
   * .
   *
   * @param msgLength 字节数组
   * @param isSeparator 是否有分隔符
   * @param separator 分隔符
   */
