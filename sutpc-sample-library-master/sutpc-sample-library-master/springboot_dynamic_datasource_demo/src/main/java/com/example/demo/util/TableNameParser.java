...
/**
 * Ultra light, Ultra fast parser to extract table name out SQLs, supports oracle dialect SQLs as
 * well.
 *
...
public final class TableNameParser {
  ...
  private static final List<String> IGNORED = Arrays
      .asList(TOKEN_PARAN_START, TOKEN_SET, TOKEN_OF, TOKEN_DUAL);
  ...
  private boolean isOracleSpecialDelete(final String currentToken, final String[] tokens,
      int index) {
  ...
  private void handleSpecialOracleSpecialDelete(final String currentToken, final String[] tokens,
      int index) {
  ...
  private boolean isCreateIndex(String currentToken, String[] tokens, int index) {
    index++; // Point to next token
    final int tokenNumber = 3;
    if (TOKEN_CREATE.equals(currentToken.toLowerCase()) && hasIthToken(tokens, index,
        tokenNumber)) {
      String nextToken = tokens[index++];
  ...
  /**
   * Retrieve all tables inside sql.
   *
