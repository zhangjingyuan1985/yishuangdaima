...
import com.sutpc.bigdata.executor.tag.hive._
import com.sutpc.bigdata.executor.tag.phoenix.{TagActivityCMExecutor, TagActivityExecutor}
...
/**
  * <p>Description:TODO 清洗原始数据到Hive指标库 </p>
...
object Hdfs2HiveJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    outputTable match {
      ...
      case "t_vehicle_gps_taxi" => new TagGpsTaxiExecutor(inputPath, zkUrl, spark, outputTable, cityPinyinCodeMapBR, encode).execute()

      case "t_vehicle_gps_bus" => new TagGpsBusExecutor(inputPath, zkUrl, spark, outputTable, cityPinyinCodeMapBR, encode).execute()
      ...
      case "t_vehicle_gps_truck" => new TagGpsTruckExecutor(inputPath, zkUrl, spark, outputTable, cityPinyinCodeMapBR, encode).execute()
