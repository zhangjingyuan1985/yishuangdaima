...
def writeConfig(tableName,firstTag,firstTagValue,secondTag,secondTagValue):
    ...
    conf.write(open("roadConfig5.ini", "w"))
    return
#读取每张表最新的标记字段
def readLocalConfig():
    conf = configparser.ConfigParser();
    conf.read("roadConfig5.ini");
...
def readDataFromOriginDatabase(TargetCursor,sql,rules,engin,tableName,tag1,tag2):
    ...
    for col in cols:
        colnum.append(col[0]);
    while True:
        data = TargetCursor.fetchmany(100000);
        if data == []:
            break;
        data = pd.DataFrame(data);
...
def run(data):
    ...
    schedule.every(1).minutes.do(doFiveMinuteUpdate,dataFor5Min);
...
if __name__ == '__main__':
    configPath = r"E:\transPass_test\road_topic\LoadDataForRoadEspecial.xlsx";
