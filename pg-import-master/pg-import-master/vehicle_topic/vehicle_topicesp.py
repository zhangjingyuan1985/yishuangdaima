...
def writeConfig(tableName,firstTag,firstTagValue,secondTag,secondTagValue):
    ...
    conf.write(open("vehicleConfigesp.ini", "w"))
    return
#读取每张表最新的标记字段
def readLocalConfig():
    conf = configparser.ConfigParser();
    conf.read("vehicleConfigesp.ini");
        ...
        if tableName in tableNameList:
            ...
            if data==[]:
               sql = sql;
                ...
                writeConfig(tableName,tag1,tagValue1,tag2,tagValue2);
        ##此代码用于清除七天以前的数据
        deleteData.deleteData(tableName,tag1,TargetCursorDes,day-7+1,TargetCnn);
        try:
...
def run(data):
    ...
    #dataForHour = data[data.导入频率=='1d'];
    #dataFor5Min = data[data.导入频率=='5m'];
    schedule.every().day.at("12:00").do(doDayUpdate,dataForDay);
    # schedule.every().friday.at("05:00").do(doWeekUpdate,dataForWeek);
    #schedule.every(5).minutes.do(doFiveMinuteUpdate,dataFor5Min);
    #schedule.every(1).hour.do(doHourUpdate,dataForHour);
    while True:
        schedule.run_pending();
        time.sleep(1);
#根据
if __name__ == '__main__':
    configPath = r"E:\transPass_test\vehicle_topic\LoadDataForVehicleTopicEsp.xlsx";
